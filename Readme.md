# Hi there, I'm Hossam Abdelmoteleb 👋

## 🚀 Quantitative Researcher | Data Scientist | ML Engineer

> Transforming complex financial data into actionable insights through advanced machine learning and quantitative strategies

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/hossamedwee/)
[![Medium](https://img.shields.io/badge/Medium-12100E?style=for-the-badge&logo=medium&logoColor=white)](https://medium.com/@hosamedwee)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:hosamedwee@gmail.com)
[![Location](https://img.shields.io/badge/Cairo-Egypt-red?style=for-the-badge&logo=googlemaps&logoColor=white)]()

---

## 💼 Professional Summary

Senior Data Scientist with **10 years of experience** leading the complete lifecycle of data science initiatives within the finance sector. I specialize in leveraging advanced machine/deep learning to extract actionable insights from complex financial data, with a proven track record in developing models for market prediction and risk analysis.

Beyond my technical contributions, I'm passionate about knowledge sharing through Medium articles and internal tutorials. I uniquely manage projects from concept to completion, including building data pipelines and deploying continuous monitoring frameworks for long-term production success.

---

## 🛠️ Technical Stack

### Programming & AI/ML
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![R](https://img.shields.io/badge/R-276DC3?style=flat-square&logo=r&logoColor=white)
![TensorFlow](https://img.shields.io/badge/TensorFlow-FF6F00?style=flat-square&logo=tensorflow&logoColor=white)
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat-square&logo=pytorch&logoColor=white)
![JAX](https://img.shields.io/badge/JAX-4285F4?style=flat-square&logo=google&logoColor=white)
![NumPy](https://img.shields.io/badge/NumPy-013243?style=flat-square&logo=numpy&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat-square&logo=mysql&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)

### Cloud & Data Engineering
![AWS](https://img.shields.io/badge/AWS-232F3E?style=flat-square&logo=amazon-aws&logoColor=white)
![GCP](https://img.shields.io/badge/GCP-4285F4?style=flat-square&logo=google-cloud&logoColor=white)
![Azure](https://img.shields.io/badge/Azure-0078D4?style=flat-square&logo=microsoft-azure&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white)
![Hadoop](https://img.shields.io/badge/Hadoop-66CCFF?style=flat-square&logo=apache-hadoop&logoColor=black)
![Kafka](https://img.shields.io/badge/Kafka-231F20?style=flat-square&logo=apache-kafka&logoColor=white)

### Data Visualization & BI
![Tableau](https://img.shields.io/badge/Tableau-E97627?style=flat-square&logo=tableau&logoColor=white)
![Power BI](https://img.shields.io/badge/Power_BI-F2C811?style=flat-square&logo=power-bi&logoColor=black)
![Plotly](https://img.shields.io/badge/Plotly-3F4F75?style=flat-square&logo=plotly&logoColor=white)
![Streamlit](https://img.shields.io/badge/Streamlit-FF4B4B?style=flat-square&logo=streamlit&logoColor=white)

---

## 🎯 Domain Expertise

- 📈 **Quantitative Strategy Development & Backtesting**
- 🤖 **Algorithmic Trading Systems**
- 🔮 **Market Prediction & Time-Series Forecasting**
- ⚠️ **Financial Risk Modeling** (Market, Credit, Operational)
- 💼 **Portfolio Optimization & Asset Allocation**
- 🧠 **Machine Learning for Alpha Generation**
- 🏗️ **End-to-End ML System Design for Finance**
- 📊 **High-Frequency Alternative Data Analysis**
- 🔄 **MLOps & Automated Model Monitoring**

---

## 💼 Professional Experience

### 🔬 Quantitative Researcher | WorldQuant (2022-2023)
*Part-Time Consultant - Remote*

- Researched and implemented quantitative trading signals (alphas) for global equity markets
- Applied advanced ML/DL and time-series analysis to vast market datasets
- Optimized signals for Sharpe ratio, turnover, and decay metrics
- Engineered diversified, uncorrelated alpha strategies

### 🔬 Quantitative Researcher | Causeway Capital (2020-2022)
*Part-Time Consultant - US Remote*

- Designed Deep Reinforcement Learning trading strategies
- Developed AI-driven strategies combining financial engineering and ML
- Deployed large-scale neural network training on cloud GPU clusters

### 📊 Data Scientist | Emaar (2016-2020)
*Dubai, UAE*

- **CLV Prediction**: Built customer lifetime value models boosting conversion rates by **15%**
- **Dynamic Pricing**: Created XGBoost pricing engine achieving **5-7% uplift** in sale prices
- **Forecasting**: Developed ARIMA/Prophet models improving planning accuracy by **20%**
- **Data Pipeline**: Architected end-to-end ETL processes for centralized data warehouse

---

## 🔬 Featured Research Projects

### 🌊 AlphaFlows
Novel framework utilizing **Generative Flow Networks (GFlowNets)** for automated discovery and diversification of formulaic alpha signals. Explores cutting-edge AI techniques beyond traditional feature engineering.

### 🔮 SigmaOracle
Deep learning framework leveraging real-time trade data to predict short-term market volatility using **LSTM networks** and novel sequential learning methodology.

### 🧬 Alpha Genesis
Dynamic neural network framework for adaptive high-frequency price prediction, addressing market non-stationarity through meta-learning. Achieved **0.66 average out-of-domain AUC**.

### 🤖 Agentic Causal Inference Framework
Generative framework for causal hypothesis and DAG discovery using **LLMs** for dynamic hypothesis evolution and enhanced validation.

---

## 🎓 Education & Certifications

**MSc, Financial Engineering (with Applied AI)** - *WorldQuant University* (2023-2025)  
**GSD, Statistical Computing** - *Cairo University* (2022)

### 📜 Certifications
- Deep Learning Specialization (Coursera)
- Data Science MicroMasters (edX)
- Advanced Data Analysis (Udacity)

---

## 📈 GitHub Stats

![Hossam's GitHub stats](https://github-readme-stats.vercel.app/api?username=HosamEdwee&show_icons=true&theme=radical)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=HosamEdwee&layout=compact&theme=radical)

---

## 📝 Latest Blog Posts

<!-- BLOG-POST-LIST:START -->
- Check out my latest articles on [Medium](https://medium.com/@hosamedwee)
<!-- BLOG-POST-LIST:END -->

---

## 🤝 Let's Connect!

I'm always open to discussing:
-  Machine Learning in Finance
-  Quantitative Trading Strategies
-  Research Collaborations
-  Exciting Opportunities

**Feel free to reach out!** 

---

⭐️ From [HosamEdwee](https://github.com/HosamEdwee)
